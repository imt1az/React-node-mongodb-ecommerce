...
<h1>hello</h1>
       ...
       <img src={renderCardImage(props.item.images)} alt="" className="md:h-96 md:w-full object-cover"/>
       </div>
          
        
        <div className="px-4 py-3  md:w-full ">
          <span className="text-blue-400 mr-3 uppercase text-xs font-bold">{props.item.brand.name}</span>
          <p className="text-lg font-bold text-black truncate block capitalize">{props.item.model}</p>
          <p className="text-lg font-semibold text-black cursor-auto my-3 text-left">${props.item.price}</p>
